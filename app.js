...
async function fetchA12() {
    ...
    try {
        const res = await fetch('https://rt.data.gov.hk/v1/transport/citybus-nwfb/route-eta/CTB/A12');
        const data = await res.json();
        ...
        if (data.data && data.data.length > 0) {
            ...
            if (airportETAs.length > 0) {
                ...
                el.textContent = minLeft === 0 ? '即將到站' : `${minLeft} 分鐘`;
                return;
            }
        }
...
async function fetch33X() {
    ...
    try {
        const res = await fetch('https://data.etabus.gov.hk/v1/transport/kmb/route-eta/33X/1');
        const data = await res.json();
